...
app.post('/api/read-file', upload.single('file'), async (req, res) => {
  try {
    ...
    if (fileType === 'text/plain' || fileType === 'text/markdown') {
      ...
      // PDF okuma için basit bir yaklaşım
      content = 'PDF dosyası okundu - içerik işleniyor...';
    } else if (fileType === 'application/msword' || fileType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      // Word dosyası okuma için basit bir yaklaşım
...
app.post('/api/generate-scenario', async (req, res) => {
  try {
    ...
    // Dosya içeriğini senaryo üretiminde kullan
    let enhancedKnowledge = knowledgeBase || {};
    if (fileContent) {
      enhancedKnowledge = {
        ...enhancedKnowledge,
        storyPatterns: [...(enhancedKnowledge.storyPatterns || []), `Dosya içeriği: ${fileContent.substring(0, 500)}...`],
        additionalContext: fileContent,
        fileBasedGeneration: true
      };
    }
